...
import { Settings2, Download, Shield, Bell, Palette, Info, Cookie, CheckCircle2, XCircle, Loader2, ExternalLink, Trash2, RefreshCw, Lock, Instagram } from "lucide-react";
...
const IG_STEPS = [
  {
    num: "1",
    label: "Install extension",
    detail: '"Get cookies.txt LOCALLY" Chrome extension install karo',
    action: { label: "Open Chrome Store →", href: EXTENSION_URL },
  },
  {
    num: "2",
    label: "Instagram par login karo",
    detail: "Apne Instagram account se sign in karo",
    action: { label: "Open Instagram →", href: "https://www.instagram.com" },
  },
  {
    num: "3",
    label: "Cookies export karo",
    detail: 'Extension icon click karo → "Export" dabao (instagram.com par)',
    action: null,
  },
  {
    num: "4",
...
function InstagramCookiesSection() {
  const [hasCookies, setHasCookies] = useState<boolean | null>(null);
  const [cookieText, setCookieText] = useState("");
  const [showInput, setShowInput] = useState(false);
  const [saving, setSaving] = useState(false);
  ...
  const looksValid = cookieText.trim().includes("instagram.com") || cookieText.trim().includes("sessionid");

  useEffect(() => {
    fetch(`${API_BASE}/cookies/instagram/status`)
      .then((r) => r.json())
      .then((d) => setHasCookies(d.hasCookies))
      .catch(() => setHasCookies(false));
  }, []);
  ...
  const saveCookies = async () => {
    ...
    setMessage(null);
    try {
      const r = await fetch(`${API_BASE}/cookies/instagram/save`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ cookies: cookieText }),
      });
      ...
      if (r.ok) {
        ...
        setShowInput(false);
        setCookieText("");
        setMessage({ type: "success", text: "✅ Instagram cookies save ho gayi! Ab public posts kaam karenge." });
      } else {
        setMessage({ type: "error", text: d.error || "Failed to save cookies." });
      }
    } catch {
  ...
  const removeCookies = async () => {
    ...
    setMessage(null);
    try {
      await fetch(`${API_BASE}/cookies/instagram/delete`, { method: "DELETE" });
      setHasCookies(false);
      setShowInput(false);
      setMessage({ type: "success", text: "Instagram cookies hata di gayi." });
    } catch {
      setMessage({ type: "error", text: "Failed to remove cookies." });
    } finally {
      setDeleting(false);
    ...
    <div className="glass rounded-2xl overflow-hidden">
      <div className="px-6 py-4 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-white text-sm">Instagram Unlock</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              Public posts, Reels & IGTV ke liye cookies zaroori hain
            </p>
          </div>
        </div>
        {hasCookies === null ? (
      ...
      {!hasCookies && (
        <div className="mx-6 mt-4 flex items-start gap-2 text-xs px-3 py-2.5 rounded-xl bg-amber-500/10 text-amber-300 border border-amber-500/20">
          <XCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
          Instagram ne 2024 se server-side access block kar diya hai. Cookies ke bina koi bhi Instagram video work nahi karega.
        </div>
      )}
      ...
      {!hasCookies && (
        <div className="px-6 py-4 space-y-3">
          ...
          <div className="space-y-2">
            {IG_STEPS.map((s) => (
              <div key={s.num} className="flex items-start gap-3 bg-white/[0.03] hover:bg-white/[0.06] rounded-xl px-4 py-3 transition-colors group">
                <span className="flex-shrink-0 w-6 h-6 rounded-full bg-pink-500/20 text-pink-400 text-xs font-bold flex items-center justify-center mt-0.5">
                  {s.num}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white font-medium">{s.label}</p>
                ...
                {s.action && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-shrink-0 flex items-center gap-1 text-xs text-pink-400 font-semibold hover:text-pink-300 transition-colors whitespace-nowrap"
                  >
                    {s.action.label}
                    <ExternalLink className="w-3 h-3" />
                  </a>
      ...
      {(!hasCookies || showInput) && (
        <div className="px-6 pb-4 space-y-2">
          <textarea
            value={cookieText}
            onChange={(e) => setCookieText(e.target.value)}
            placeholder={"# Netscape HTTP Cookie File\n# instagram-cookies.txt ka content yahan paste karo"}
            className={`w-full h-28 text-xs font-mono bg-black/40 border rounded-xl px-3 py-3 text-white/80 placeholder-white/20 focus:outline-none focus:ring-2 resize-none transition-colors ${
              cookieText && looksValid
                ? "border-green-500/40 focus:ring-green-500/30"
                : cookieText && !looksValid
                ? "border-red-500/30 focus:ring-red-500/20"
                : "border-white/15 focus:ring-pink-500/40"
            }`}
          />
          {cookieText && !looksValid && (
            <p className="text-xs text-red-400/80 flex items-center gap-1">
              <XCircle className="w-3 h-3" /> Yeh Instagram cookies nahi lagte — instagram.com se export karo
            </p>
          )}
          {cookieText && looksValid && (
            <p className="text-xs text-green-400/80 flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3" /> Valid Instagram cookies detect hui — save kar sakte ho
            </p>
          )}
          <button
            onClick={saveCookies}
            disabled={saving || !cookieText.trim() || !looksValid}
            className="w-full text-sm bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white rounded-xl px-4 py-2.5 font-semibold transition-all disabled:opacity-40 flex items-center justify-center gap-2"
          >
            {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving...</> : "Save Cookies & Unlock Instagram"}
          </button>
        </div>
      )}
    ...
    <div className="container mx-auto px-4 py-20 max-w-2xl">
      ...
      >
        ...
        <div className="space-y-6">
          {/* Instagram Cookies — required for Instagram to work */}
          <InstagramCookiesSection />

          {/* YouTube Cookies — for bot-blocked / age-restricted */}
          <CookiesSection />
